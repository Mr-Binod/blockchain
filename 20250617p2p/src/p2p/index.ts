...
class P2PNode implements IP2PNode {
    ...
    constructor(port : number, peers : string[] = []) {
        ...
        // http 서버 포트 지정 대기상태
        this.httpPort = port + 1000;
        // 서버 대기상태 로직

        peers.forEach((peer) => this.connectToPeer(peer));

        this.initHTTPServer();
        console.log("WebSocketServer 대기상태 " + port + "번 대기중");
        console.log("HTTPServer 대기상태 " + this.httpPort + "번 대기중");
    }
    ...
    initConnection(ws : WebSocket) {
        ...
        ws.on("message", (msg) => this.handlerMessage(ws, msg.toString())) // 메시지 처리 로직
    ...
    // 내가 가지고 있는 체인의 정보를 요청한 피어에게 체인전달.
...
const peers = ["ws://192.168.0.75:3000", "ws://192.168.0.5:3000" ]
const peers2 = ["ws://192.168.0.75:3001",  ]
// const peers3 = ["ws://192.168.0.5:3000", "ws://192.168.0.235:3000" ]
const p2pNode = new P2PNode(3000, peers);
const p2pNode2 = new P2PNode(3001, peers2);
// const p2pNode3 = new P2PNode(3002, peers3);
initBalance(p2pNode)
// initBalance(p2pNode3)
initBalance(p2pNode2)
// 랜선 ws://192.168.0.99:3000
// 공유기  