...
describe("block 테스트", () => {
    ...
    test("블록 생성", () => {
        ...
        block = new block_1.Block("1.0", 0, 0, "0".repeat(64), 1, data);
        block.mine();
    ...
    test(`block 체굴 혹은 마이닝`, () => {
        for (let i = 1; i < 1000; i++) {
            console.log(`${i} 번째 블록 마이닝 시작`);
            console.time("block mine...");
            const data = [`block ${i}tx`];
            const previousBlock = blockChain[i - 1];
            const block = new block_1.Block("1.0", i, new Date().getTime(), previousBlock.hash, previousBlock.difficulty, data);
            block.mine();
