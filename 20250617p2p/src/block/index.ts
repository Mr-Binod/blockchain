...
export class Block implements IBlock {
    ...
    static createHash (
        ...
        // 해시값을 구해서 해시 문자열로 반환
        return SHA256(`${version}${heigth}${timestamp}${previousHash}${merkleRoot}${difficulty}${nonce}`).toString();
    }
    ...
    static isValidBlock (block : Block, previousBlock : Block) : boolean {
        ...
        // 블록 해시 다시 검증
        const blockHash = Block.createHash(
           block.version, 
           block.height, 
           block.timestamp,
           block.previousHash, 
        ...
        if(blockHash !== block.hash) return false;
