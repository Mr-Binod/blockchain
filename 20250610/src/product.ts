...
// 판매금액을 행사 기간이어서 모든 상품이 2000 원 씩 할인상태야 퍼센트를 적용한 금액
class FlatPercent implements IDiscount {
    amount : number;
    percent : number;
    constructor(amount : number, percent : number) {
...
class ProductDisCount {
    product : Products // 클래스로 만든 인스턴스를 할당할 변수
    discount : IDiscount // 할인의 인스턴스의 푀소의 기본형태를 할당할 변수
