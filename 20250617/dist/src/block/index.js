...
class Block {
    ...
    static createHash(version, heigth, timestamp, previousHash, difficulty, merkleRoot, nonce) {
        // 해시값을 구해서 해시 문자열로 반환
        return (0, crypto_js_1.SHA256)(`${version}${heigth}${timestamp}${previousHash}${merkleRoot}${difficulty}${nonce}`).toString();
    }
    // 
    ...
    mine() {
        ...
        while (true) {
            this.nonce++;
            this.hash = Block.createHash(this.version, this.height, this.timestamp, this.previousHash, this.difficulty, this.merkleRoot, this.nonce);
    ...
    static isValidBlock(block, previousBlock) {
        // 블록 높이 검증
        console.log(block.height, 'ss', previousBlock.height);
        ...
        if (block.previousHash !== previousBlock.hash)
            return false;
        // 블록 해시 다시 검증
        const blockHash = Block.createHash(block.version, block.height, block.timestamp, block.previousHash, block.difficulty, block.merkleRoot, block.nonce);
