...
contract Baseball {
    ...
    // wei 단위가 아닌 ether 다뉘로 표현을 할때 사용
    uint private ticket = 0.01 ether;

    // 컴퓨터가 생성할 야구 숫자 번호 (3자리 숫자) (100 ~ 999)
    // 숫자를 따로따로 저장하는게 로그를 찍을때 S B O 상태변수를 3 가지
    uint private random;
    ...
    constructor() {
        owner = msg.sender;
        // 랜덤 값을 만들어야함
        // 랜덤 해시 값ㅇ르 만들어서 정수로 변환
        // keccak256(x) 해시값을 만드는 함수
        // 솔리디티에서 랜덤값ㅇ르 뽑을 많이 사용한다 매개변수를 sha-3 의 해시값으로
        // 매개변수로 전달한 값을 바이트 배열로 만들어주는 함수
        // abi.encodePacked(arg); 바이트 문자열 배열
    ...
    function gameStart(uint count) external payable {
        ...
        require(msg.value == ticket, "unsufficient fund");
    ...
    // 상위 컨트랙트
    // 새로운 컨트랙트 주소를 계속 만들어서 게임 방 만드는것처럼 
    // 컨트랙트가 컨트랙트를 제어
}